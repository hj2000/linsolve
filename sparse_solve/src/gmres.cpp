            ...
            double tol, int maxit )
{
    ...
    gsl_vector*  r     = gsl_vector_alloc( n );
    gsl_vector*  w_bar = gsl_vector_alloc( n );
    gsl_blas_dcopy( b, r );
    gsl_spblas_dgemv( CblasNoTrans, -1.0, A, x, 1.0, r );
    double      beta = gsl_blas_dnrm2( r );
    gsl_matrix* V    = gsl_matrix_alloc( maxit + 1, n );
    ...
    for ( int i = 0; i < maxit; i++ )
    {
        auto w = gsl_matrix_row( V, i + 1 );
        auto v = gsl_matrix_row( V, i );
        gsl_precondition( pred, &v.vector, w_bar );
        gsl_spblas_dgemv( CblasNoTrans, 1.0, A, w_bar, 0.0, &w.vector );
        for ( int j = 0; j <= i; j++ )
        {
            auto   vj = gsl_matrix_row( V, j );
            double h  = 0.0;
    ...
    {
        auto vi = gsl_matrix_row( V, i );
        gsl_precondition( pred, &vi.vector, w_bar );
        gsl_blas_daxpy( gsl_vector_get( &me.vector, i ), w_bar, x );
    }
    gsl_matrix_free( H );
    gsl_matrix_free( V );
    gsl_vector_free( r );
